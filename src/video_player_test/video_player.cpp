...
using AVPacketDeleter = decltype([](AVPacket *pkt) { av_packet_free(&pkt); });
...
struct PacketQueue {
  std::deque<MyAVPacketList> pkt_list;
  int size = 0;
  int64_t duration = 0;
  int abort_request = 1;
  int serial = 0;
  ...
  int put_private(std::unique_ptr<AVPacket, AVPacketDeleter> pkt) {
    if (abort_request) {
      return -1;
    } else {
      size += pkt->size + sizeof(MyAVPacketList);
      duration += pkt->duration;
      pkt_list.emplace_back(pkt, serial);
      cond.notify_one();
      return 0;
    }
  }

  int put(std::unique_ptr<AVPacket, AVPacketDeleter> pkt) {
    AVPacket *pkt1;
    int ret;
    ...
    {
      auto lock = std::scoped_lock{q->mutex};
      ret = put_private(q, pkt1);
    }
  ...
  static int put_nullpacket(PacketQueue *q, AVPacket *pkt,
                                         int stream_index) {
...
using AVFrameDeleter = decltype([](AVFrame *frame) { av_frame_free(&frame); });
...
struct Frame {
  ...
  void unref() { av_frame_unref(frame.get()); }
};
...
struct FrameQueue {
  ...
  FrameQueue(PacketQueue *pktq) : pktq{pktq} {}

  ~FrameQueue() {
    while (!queue.empty()) {
      frame_queue_unref_item(queue.front());
      queue.pop();
    }
  }

  void signal() {
    auto lock = std::scoped_lock{f->mutex};
    f->cond.notify();
  }
};
...
using AVPacketDeleter = decltype([](AVPacket *pkt) { av_packet_free(&pkt); });
using AVCodecContextDeleter = decltype([](AVCodecContext *avctx) {
...
struct Decoder {
  std::unique_ptr<AVPacket, AVPacketDeleter> pkt;
  PacketQueue *queue;
  std::unique_ptr<AVCodecContext, AVCodecContextDeleter> avctx;
  int pkt_serial;
  int finished;
  int packet_pending;
  ...
  Decoder(AVCodecContext *avctx, PacketQueue *queue,
          std::condition_variable &empty_queue_cond)
      : pkt{av_packet_alloc()}, avctx{avctx}, queue{queue},
        empty_queue_cond{empty_queue_cond}, start_pts{AV_NOPTS_VALUE},
        pkt_serial{-1} {}
};

struct VideoState {
  std::thread read_tid;
  AVInputFormat const &iformat;
  int abort_request;
  int force_refresh;
  int paused;
  int last_paused;
  ...
  VideoState(std::string filename, AVInputFormat const &iformat)
      : filename{filename}, iformat{iformat}, pictq{&videoq}, sampq{&audioq} {}
};
...
inline constexpr auto pixel_format_thingy = AV_PIX_FMT_BGR32_1;
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block,
  ...
  auto lock = std::unique_lock{q->mutex};
...
[[deprecated]] static VideoState *stream_open(const char *filename,
