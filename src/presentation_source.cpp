...
void convert_slide(Magick::Image &img, triple_buffer::buffer &buffer,
  ...
  try {
    ...
    bg_colour.alpha(0);
    img.extent({1920, 1080}, bg_colour, Magick::CenterGravity);
...
public:
  ...
  http_delegate(std::vector<triple_buffer::buffer> &slides,
                std::vector<thumbnail> &thumbnails, std::size_t &active_slide,
                WriteFrame const &write_frame)
      : slides{slides}, thumbnails{thumbnails}, active_slide{active_slide},
...
<html>
  ...
  <body>
    Presentation
    <br/>
    <input type="file" onchange="fetch('/upload', {{method: 'POST', body: event.target.files[0]}})">
      Upload
    </input>
    <br/>
    <button
      onclick="window.parent.postMessage({{msg: 'show_detail_view', data: `http://${{window.location.host}}/details`}}, '*')"
    >
      Control slides
    </button>
    ...
    <script>
      ...
      open_ws();
          ...
          "active_slide"_a = active_slide + 1,
...
<html>
  ...
  auto blob = Magick::Blob{};
  {
    auto &body = req.body();
    auto size = body.size();
    auto data = make_unique_for_overwrite<char[]>(size);
    beast::error_code ec;
    body.file().read(data.get(), size, ec);
    if (ec) {
      return send(http::bad_request(req, "Cannot read body"));
    }
    blob.updateNoCopy(data.release(), size);
  }
  */
        auto blob = Magick::Blob{req.body().data(), req.body().size()};
...
int main([[maybe_unused]] int argc, char **argv) {
  ...
  auto http_delegate_ = std::make_shared<http_delegate<decltype(write_frame)>>(
      slides, thumbnails, active_slide, write_frame);
